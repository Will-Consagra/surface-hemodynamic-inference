...
MODELDIR = "../PreTrainedModels/models/summary_nets_2param_basis"
LIKEMULDIR = "../PreTrainedModels/models/lik_emuls_2param_basis"
fname_summary_net_state_dict = os.path.join(MODELDIR, "posterior_mean_summary_statistic_2param_hrf.pth")
...
summary_network.load_state_dict(torch.load(fname_summary_net_state_dict)["model_state_dict"])
summary_network = summary_network.to(device)
summary_network.eval()
for param in summary_network.parameters():
...
Nbatch = 100
learning_rate = 1e-5
optim_lik = torch.optim.Adam(params=lik_emul.parameters(), 
								lr=learning_rate)
...
for epoch in range(num_epochs):
	...
	## log-likelihood 
	log_lik = lik_emul(batch_theta_tildes.to(device)).log_prob(batch_S_y)
	...
	if epoch % 10000 == 0:
		...
		with torch.no_grad():    
			...
			test_loss = -lik_emul(theta_tildes_test.to(device)).log_prob(test_S_y).mean()
			# eval 
			print(f"Epoch {epoch}, Test Loss: {test_loss.item():.4f}")
			# inf 
			test_S_y_samples = lik_emul(theta_tildes_test.to(device)).sample((Npost_samples,))
			test_S_y_errors = torch.mean((test_S_y_samples.mean(dim=0) - test_S_y)**2,dim=0)
			tl1 = test_S_y_errors[0].item()
			tl2 = test_S_y_errors[1].item()
		fname_lik_model_state_dict = os.path.join(LIKEMULDIR, "likelihood_emulator_posterior_mean_summary_2param_hrf_%s.pth" % (epoch,))
		torch.save({
					"epoch": epoch,
					"summary_network_fname": fname_summary_net_state_dict,
					"test_neg_lik":test_loss.item(),
					"test_losses": (tl1, tl2),
					"model_state_dict": lik_emul.state_dict(),
					"optimizer_state_dict": optim_lik.state_dict(),
					}, fname_lik_model_state_dict)
