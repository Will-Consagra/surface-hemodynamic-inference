...
def np2freq(sigs, bpfilter=False):
	sigs_ft = torch.fft.rfft(sigs, dim=-1)
	if bpfilter:
		sigs_ft = sigs_ft[:,bp_filter]
...
## bp-filter (optional)
nyquist_freqs = torch.fft.rfftfreq(M, d=t_r)
f_lo = 0.01; f_hi = 0.1;
bp_filter = ((nyquist_freqs >= f_lo) & (nyquist_freqs <= f_hi))
BP_FILTER = True 
...
## FT 
y_test_eps_freq = np2freq(y_eps_test, bpfilter=BP_FILTER)
...
if BP_FILTER:
	input_size = bp_filter.sum().item()
else:
	input_size = M_record//2 + 1
	#hidden_sizes = [M_record, M_record//2, M_record//4, M_record//16]
...
for epoch in range(num_epochs):
	...
	## FT 
	batch_y_eps_freq = np2freq(batch_y_eps, bpfilter=BP_FILTER)
	## S(y)
	...
	optim.step()
	#if epoch % 10000 == 0:
	#print(f"Epoch {epoch}, Loss: {loss.item():.4f}, Time: {time.time()-t1:.4f}")
	t1 = time.time()
	with torch.no_grad():            
		test_S_y = summary_network(y_test_eps_freq.to(device))
		test_loss = torch.mean((test_S_y - theta_tildes_test.to(device))**2, dim=0).cpu().detach()
		tl1 = test_loss[0].item()
		tl2 = test_loss[1].item()
		# eval 
		print(f"Epoch {epoch}, Test Loss theta-1: {tl1:.4f}, Test Loss theta-2: {tl2:.4f}")
		fname_summary_net_state_dict = os.path.join(MODELDIR, "posterior_mean_summary_statistic_2param_hrf_epoch_%s_%s.pth" % (HRFMODEL, epoch))
		torch.save({
					"epoch": epoch,
					"test_losses": (tl1, tl2),
